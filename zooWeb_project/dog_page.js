//Reading and visualizing JSON data
...
async function fetchProductsData() {
  ...
  function sortByProductNameAZ(products) {
    ...
    async function updateProductList() {
      const productBrandFilter = document.getElementById("productBrandFilter").value;
      ...
      let filteredProducts = productsData.filter((product) => {
        return (
          (productBrandFilter === "" || product.productBrand === productBrandFilter) &&
          (ratingFilter === "" || parseInt(product.rating) === parseInt(ratingFilter)) &&
      ...
      const startIndex = productListDiv.children.length;
      const newProducts = sortedProducts.slice(startIndex, startIndex + numItemsToShow);
    ...
    newProducts.forEach((product) => {
      ...
      <div class="product-item">
        ...
        <h3 class="product-heading">${product.productBrand} ${product.productName}</h3>
      ...
      loadMoreBtn.style.display = totalProductsDisplayed < totalProductsAvailable ? "block" : "none";
  ...
  async function populateFilters() {
    ...
    productBrandFilter.innerHTML = '<option value="" class="filter-option">All</option>';
    ratingFilter.innerHTML = '<option value="" class="filter-option">All</option>';
    typeFoodFilter.innerHTML = '<option value="" class="filter-option">All</option>';
  
    const uniqueBrands = [...new Set(productsData.map((product) => product.productBrand))];
    const uniqueRatings = [...new Set(productsData.map((product) => product.rating))];
    const uniqueTypeFoods = [...new Set(productsData.map((product) => product.typeFood))];
  ...
  document.getElementById("productBrandFilter").addEventListener("change", updateProductList);
  document.getElementById("ratingFilter").addEventListener("change", updateProductList);
  document.getElementById("typeFoodFilter").addEventListener("change", updateProductList);
  document.getElementById("loadMoreBtn").addEventListener("click", updateProductList);
