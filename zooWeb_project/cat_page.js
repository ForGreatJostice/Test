...
async function fetchProductsData() {
  try {
    const response = await fetch("cat_food_DATA.json");
...
//Product count
  ...
  loadMoreBtn.style.display =
    totalProductsDisplayed < totalProductsAvailable ? "block" : "none";

  if (startIndex + numItemsToShow >= sortedProducts.length) {
    const loadMoreBtn = document.getElementById("loadMoreBtn");
...
document
  ...
  .addEventListener("click", updateProductList);
